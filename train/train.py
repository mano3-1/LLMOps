...
from unsloth import fix_untrained_tokens
...
def parse_arge():
    ...
    parser.add_argument("--save_steps", type=int, default=4, help="number of training steps before saving a checkpoint")
    parser.add_argument("--logging_steps", type=int, default=4, help="number of steps before logging the training metrics")
    ...
    parser.add_argument("--sm_train_dir", type=str, default="/opt/ml/input/data/train")
    parser.add_argument("--sm_validation_dir", type=str, default="/opt/ml/input/data/test")
    parser.add_argument("--model_eval_s3_loc", type=str, default="")
    parser.add_argument("--run_experiment", type=str, default="True")
    parser.add_argument("--hf_token", type=str, default="None")
    parser.add_argument("--unsloth", type=str, default="False")
...
def load_pretrained_model_and_tokenizer_unsloth(args, model_name, compute_dtype):
...
def load_pretrained_model(args, model_name, compute_dtype):
    ...
    #     print(param)
    #     if hasattr(param, 'qint4'):
    #         print(f"Parameter {name} is quantized to 4-bit.")
...
def load_from_disk(directory, test=False):
    files = os.listdir(directory)
    files = [_ for _ in files if _.split(".")[-1] == "parquet"]
    assert len(files)!=0, "No parquet file is found"
    assert len(files)==1, "There are more than one parquet file found.."
    
    file = os.path.join(directory, files[0])
    if test:
        df = load_dataset("parquet", data_files={"test":file}, split="test")
    else:
        df = load_dataset("parquet", data_files={"train":file}, split="train")
    return df

def training_function(args):
    print(f"merging weights: {args.merge_weights}")
    train_dataset = load_from_disk(args.sm_train_dir)
    if args.sm_validation_dir!="None":
        eval_dataset = load_from_disk(args.sm_validation_dir, test=True)
    else:
        eval_dataset = None
    ...
    if args.unsloth=="True":
        model,tokenizer = load_pretrained_model_and_tokenizer_unsloth(args, model_name, compute_dtype)
        ...
        print(f"found lora modules: {lora_modules}")
